...
export function humanizeProviderError(err, providerName, model) {
  ...
  if (/CORS/i.test(msg)) {
    if (isOWUI) {
      return `${providerName} blocked the request (CORS). Either enable CORS on your Open WebUI server (set CORS_ALLOW_ORIGIN to "*" or include "moz-extension://*"), or grant host permission to your server URL in about:addons → ScreenSense → Permissions.`;
    }
  ...
  if (/Failed to fetch|NetworkError|TypeError|fetch failed/i.test(msg)) {
    if (isOWUI) {
      ...
      if (isHttpUrl) {
        lines.push(
          "",
          "Most common cause: Firefox MV3 extensions are configured to upgrade http:// to https:// by default, which breaks plain-http servers like a local Open WebUI on http://localhost. This build of ScreenSense overrides that — but if you're still seeing this error after reinstalling v0.3+, also check:",
        );
      } else {
        lines.push("", "Most likely causes:");
      }
      lines.push(
        " • Host permission not granted — about:addons → ScreenSense → Permissions → enable \"Access your data for all websites\".",
