...
els.outputMode.addEventListener("change", () => {
  // Update the visible fields immediately; persistence happens on Save.
...
els.customizeHotkeys.addEventListener("click", async () => {
  // chrome://extensions/shortcuts can't be opened with a normal link from an
  // extension page, but tabs.create is allowed.
  ...
  try {
    ...
    setStatus("info", "Open " + url + " to customize hotkeys.");
  }
});
...
els.actionsSection.addEventListener("click", async (e) => {
  ...
  // Restricted URLs that can't have content scripts. Covers both Firefox
  // (about:, moz-extension:) and Chromium (chrome:, edge:, chrome-extension:,
  // and the Chrome Web Store, which Chromium blocks injection into).
  if (/^(about:|moz-extension:|chrome:|edge:|chrome-extension:|view-source:|resource:|devtools:)/.test(url)
      || /^https?:\/\/(addons\.mozilla\.org|accounts-static\.cdn\.mozilla\.net|chromewebstore\.google\.com|chrome\.google\.com\/webstore)/.test(url)) {
    setStatus("error", "This page is restricted by the browser (e.g. browser settings pages or the extension store). Try a regular http(s) page.");
    return;
  }
  ...
  try {
    ...
    if (/Missing host permission/i.test(m)) {
      throw new Error(
        ...
        "Open chrome://extensions → ScreenSense → Details → \"Site access\" and set it to \"On all sites\", then close and reopen this popup. (On Edge use edge://extensions.)"
      );
    }
...
diagBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (/^(about:|moz-extension:|chrome:|edge:|chrome-extension:|view-source:|resource:|devtools:|file:)/.test(tab.url || "")) {
  ...
  try {
    ...
    if (!granted) {
      diagLog("    open chrome://extensions → ScreenSense → Details → Site access");
      diagLog("    set Site access to \"On all sites\".");
    }
  } catch (e) {
    ...
    if (baseUrl) {
      ...
      try {
        ...
        if (!granted) {
          ...
          diagLog("    chrome://extensions → ScreenSense → Details → Site access");
          diagLog("    set Site access to \"On all sites\".");
        }
      } catch (e) {
...
reloadContentBtn?.addEventListener("click", async () => {
  ...
  try {
    ...
    if (/Missing host permission/i.test(String(e?.message || ""))) {
      diagLog("    Set Site access to \"On all sites\" in chrome://extensions → ScreenSense → Details.");
    }
  }
});
