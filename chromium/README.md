# ScreenSense — Chromium Extension (Chrome / Edge / Opera / Brave)

A Chromium port of the [ScreenSense](../PROJECT_STATE.md) desktop app. Same four AI providers (Anthropic Claude, OpenAI ChatGPT, Google Gemini, Open WebUI), same quiz/ask modes, same auto-highlight-on-screen behavior. Built from the same codebase as the Firefox build (v1.4.0), with the manifest and a handful of browser-specific strings adapted for Chromium's MV3 service-worker model.

> **Looking for the Firefox version?** Use the `screensense-firefox` build instead — the two differ in their `background` declaration and a few browser-specific UI strings.
...
- **Full-screen ask** — generic Q&A about whatever's on screen, streamed into a draggable panel.
- **Region ask** — same, but for a cropped region.
...
- **Master on/off toggle** — flip the switch in the popup to silence everything without uninstalling. The toolbar icon shows an "off" badge.

### Hotkeys

| Shortcut | Action |
|---|---|
| `Ctrl+Shift+U` | Ask about full screen |
| `Ctrl+Shift+Y` | Quiz mode (full screen) |
| `Ctrl+Shift+E` | Ask about region |
| (unbound) | Quiz mode (region) — bind at `chrome://extensions/shortcuts` |
| (unbound) | Toggle on/off |

> Chromium reserves `Ctrl+Shift+Space` and several other chords, so the defaults differ from the Firefox build. All shortcuts are rebindable at `chrome://extensions/shortcuts` (Edge: `edge://extensions/shortcuts`).
...
1. Unzip the build somewhere permanent (don't delete the folder afterward — Chromium loads it from disk every launch).
...
The extension loads immediately and **persists across browser restarts** — unlike Firefox's temporary add-ons, Chromium keeps unpacked extensions loaded until you remove them.

### Grant host access (REQUIRED for Open WebUI and non-active-tab features)

By default the extension may install with access limited to the current tab. To talk to a self-hosted Open WebUI server, use the auto-watcher, or use hotkeys on background tabs, grant full site access:

1. Open `chrome://extensions`
2. Find **ScreenSense**, click **Details**
3. Under **Site access**, choose **On all sites**

Without this, action buttons may fail and Open WebUI requests will report "Couldn't reach Open WebUI."

### Pin the toolbar icon (optional)

Click the puzzle-piece **Extensions** button in the toolbar, then the pin icon next to ScreenSense, so the icon is always visible.

### Reload any tabs that were open before install

Chromium only injects content scripts into tabs that load *after* the extension is installed. If the quiz/ask buttons do nothing on a tab you had open, refresh that tab. The extension self-heals by injecting on demand where it has access, but a manual reload is faster.
...
- **Base URL wrong** — paste it into a normal browser tab and confirm it loads.
- **Self-signed cert** — visit the URL once in a normal tab and accept the cert so Chromium trusts it.
- **CORS blocked** — your Open WebUI server needs `CORS_ALLOW_ORIGIN` set to `*` (or to `chrome-extension://*` for paranoid setups).
...
- No installer / USB wizard — Chromium extensions install via the Web Store or **Load unpacked** in `chrome://extensions`.
...
- No `faulthandler` / file logging — service-worker errors land in the extension's console (`chrome://extensions` → ScreenSense → **Service worker** link → Console).
...
- **MV3 service-worker lifetime**: Chromium terminates idle service workers after 30s. Long-running operations (the SSE stream for Ask, and quiz generation) keep the worker alive via an active port plus a periodic `storage.session` write (see `startHeartbeat` in `background.js`). If the worker is forcibly terminated mid-stream anyway, the panel shows whatever streamed before the cutoff.
- **`captureVisibleTab` rate limit**: ~2 calls/sec. The auto-watcher's 2.5-second debounce stays comfortably under this.
- **Restricted pages**: Chromium blocks content scripts on `chrome:`, `edge:`, `chrome-extension:`, `devtools:`, and the Chrome Web Store. Hotkeys and the popup actions won't work there.
...
2. **No Ask follow-up turn** — desktop supports continued conversation in the same panel; browser version is single-turn. Adding it is straightforward: keep a conversation array in the panel state and forward to `provider.stream()` with the full history.
3. **Pixel-fallback highlight** — when DOM matching fails, we currently skip the highlight. Could draw a box at LLM-reported image coords (mirrors desktop's OCR-coord fallback), but most LLMs don't return per-answer pixel coords reliably. Would need a second prompt or a different LLM call shape.
4. **No `tests/` directory** — same gap as the desktop project. Smoke tests for `providers.js` and the JSON parser would be cheap and catch regressions.
